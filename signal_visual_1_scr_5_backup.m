% script # 5 for signal_visualization program
% plots the selected waveform and allows to
% select the limits for function overcomplete set
% creation

% updated 7.10.2004 15-26, 
% updated 30.01.2005 15-37 
% updated 03.04.2005 8-12 -- set with taking into account TREND in the signal

% recalled from scr_3 or from wave_composer_1
...
%wf=abs(sig(wf_beg:wf_end))/max(abs(sig(wf_beg:wf_end)));% norming initial function
 % 
 %
 %
 %
...
set(lchd2,'ydata',wf,...
   ...
   'buttondownfcn','',...
   'userdata','');
set(ad2,'xlim',[1 wf_end-wf_beg+1],'xtick',linspace(0,(wf_end-wf_beg+1),5),...%[1:2:wf_end-wf_beg+1],...
...
rs_e=.7;
rs=[rs_b:rs_s:rs_e];
rs2=uicontrol('style','edit','units','normalized',...
   'position',[.62 .81 .1 .04],'fontsize',10,...
   'string',num2str(rs_b),'createfcn','',...
   'callback','rs_b=str2num(get(gco,''string''));rs=[rs_b:rs_s:rs_e];');
rs3=uicontrol('style','edit','units','normalized',...
   'position',[.73 .81 .1 .04],'fontsize',10,...
   'string',num2str(rs_s),'createfcn','',...
   'callback','rs_s=str2num(get(gco,''string''));rs=[rs_b:rs_s:rs_e];');
rs4=uicontrol('style','edit','units','normalized',...
   'position',[.84 .81 .1 .04],'fontsize',10,...
   'string',num2str(rs_e),'createfcn','',...
   'callback','rs_e=str2num(get(gco,''string''));rs=[rs_b:rs_s:rs_e];');
rs1=uicontrol('style','text','units','normalized',...
   'position',[.51 .81 .1 .04],'fontsize',10,...
   'string','Rises:',...
   'createfcn','');
...
% rows for polynomial trend in the form a+bx, a=0 (we'll take absolute term in the "rise/low")
% for "b" : 
Bs_b=-.3;
Bs_s=1e-1;
Bs_e=.3;
...
Bs1=uicontrol('style','text','units','normalized',...
   ...
   'string','Poly, b :',...
   'tooltip','The set for linear term "b" in the polinomial TREND accounting in the form y=0+bx.');
...
%Cs_e=0;
%Cs=[Cs_b:Cs_s:Cs_e];
%Cs2=uicontrol('style','edit','units','normalized',...
%   'position',[.62 .685 .1 .035],'fontsize',10,...
%   'string',num2str(Cs_b),'createfcn','',...
%   'callback','Cs_b=str2num(get(gco,''string''));Cs=[Cs_b:Cs_s:Cs_e];');
%Cs3=uicontrol('style','edit','units','normalized',...
%   'position',[.73 .685 .1 .035],'fontsize',10,...
%   'string',num2str(Cs_s),'createfcn','',...
%   'callback','Cs_s=str2num(get(gco,''string''));Cs=[Cs_b:Cs_s:Cs_e];');
%Cs4=uicontrol('style','edit','units','normalized',...
%   'position',[.84 .685 .1 .035],'fontsize',10,...
%   'string',num2str(Cs_e),'createfcn','',...
%   'callback','Cs_e=str2num(get(gco,''string''));Cs=[Cs_b:Cs_s:Cs_e];');
%Cs1=uicontrol('style','text','units','normalized',...
%   'position',[.51 .685 .1 .035],'fontsize',10,'fontweight','bold',...
%   'string','Poly, c :',...
%   'tooltip','The set for quadratic term "c" in the polinomial TREND accounting in the form a+bx+cx^2.');
...
sfd2=uicontrol('Style','pushbutton','Units','normalized',...
   ...
   'Callback','set(itd2,''ForegroundColor'',[1 0 0],''string'',''Wait for set functions to be created...'',''visible'',''on''); pause(.1); F=funsetcreation(wf,str2num(get(lfs2,''string'')),sc,dl,0,rs);set(itd2,''string'',''Functions are ready.'',''ForegroundColor'',[0 1 0]);[str,stb]=size(F);   str_cntr=0;    set(lchd2,''Color'',[1 0 0]);');

%text for file name with the set to open
sfot=uicontrol('style','edit','units','normalized','FontWeight','bold',...
   'position',[.505 .51 .17 .05],'fontsize',10,...
   'createfcn','');

% button for opening the set from file
uicontrol('style','pushbutton','units','normalized','FontWeight','bold',...
   'position',[.505 .45 .2 .05],'fontsize',10,...
   'createfcn','','string','Load set from file',...
   'callback','load(get(sfot,''string''));str_cntr=0;[str stb]=size(F);');
